...
- Claim Lane - the lane used for claim visuals. Claims in a claim lane are ordered along the lane axis.
...
- sourcesScale - the owned scale value for a target claim's source side. It is stored on the target claim so every source of that target shares the same scale, and the planner propagates that value to the appropriate source-side visuals.
  The allocation is recursive. Direct confidence children divide that target-owned budget, their widths add back up to the target's source-side width, and direct relevance children inherit the affected target-side budget unchanged while still reweighting the affected confidence child's share.
