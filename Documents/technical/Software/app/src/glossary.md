...
- Aggregator - a display shape representing where source confidences or relevances combine to produce the score of the target claim. An aggregator can still exist in the snapshot even when the display does not need to show it. When shown, it attaches flush to a side of its target, matches the length of that target side, and extends outward from that side. A delivery aggregator attaches to the relevant side of a claim. A relevance aggregator attaches to the relevant side of a junction and is the target shape for incoming relevance connectors. Aggregators with zero or one incoming connectors can remain collapsed to zero depth.
