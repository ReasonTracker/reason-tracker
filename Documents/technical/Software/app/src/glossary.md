...
- sourcesScale - the owned potential scale value for a target claim's source side. The target claim owns that fixed source-side budget, and the planner propagates the resulting child potential scales to the appropriate source-side visuals.
  Direct confidence children of the same target start from equal inherited shares of that target-owned potential scale before any relevance modifiers are applied. Direct relevance children inherit the affected confidence connection's potential scale unchanged. Relevance can increase or decrease the affected confidence child's share of that fixed target-owned potential scale, while current score still controls only the fluid fraction inside that full pipe size.
