...
- The planner owns positions, scales, target-stack offsets, reveal tracks, and each confidence connection's junction span. `@reasontracker/components` owns attachment ports, route geometry, junction polygons, aggregators, and SVG rendering from each resolved scalar frame.
  - During `voila`, the planner transitions claims, connectors, junctions, and target-stack offsets to the post-command structural layout while retaining pre-command score values. This establishes room for a newly added connector before it reveals during `sprout`. `firstFill` reveals the new connector's fluid, and `wave` remains the separate score-propagation phase.
