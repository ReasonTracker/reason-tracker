...
- Connector route centers and connector junction positions are layout output.
...
- Actual confidence widths are connector world-unit measurements computed from each connector span's scale and connector confidence.
- Potential pipe widths are connector world-unit measurements computed from each connector span's scale.
- Claim height and the confidence source span's potential width use the same base height and the same source span scale.
...
- A confidence connector without attached relevance uses one delivery span from source claim to target claim.
- The source span uses the source score's source scale.
- The delivery span uses the source score's delivery scale.
- The delivery scale is the source scale multiplied by the source score's relevance.
- Full pro relevance uses a `2x` delivery multiplier.
- Full con relevance uses a `0.5x` delivery multiplier.
- Relevance changes resize the delivery span without resizing the source claim or the source span.
...
- A relevance connector enters the connector junction along the tapered top edge when its source claim is above the junction and along the tapered bottom edge when its source claim is below the junction.
- The targeted confidence connector is rendered as two pipe spans: source span into the connector junction, then delivery span from the connector junction to the target claim.
...
- At 100% connector confidence, each rendered actual confidence width matches that span's rendered potential pipe width because both are calculated from the same base node height and span scale.
