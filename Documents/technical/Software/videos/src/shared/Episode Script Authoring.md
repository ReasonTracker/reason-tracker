...
Display spoken text for the specified duration. Captions are fixed to the composition and remain in place while the scene camera moves. Omit `position` for the lower-third placement; set `position` to `"center"` to center the caption on screen. Use a nonblocking action with a negative offset to align a caption with a preceding graph animation without advancing the timeline.
