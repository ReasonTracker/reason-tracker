...
type EpisodeBrandSequenceContentProps = Pick<EpisodeBrandSequenceProps, "durationInFrames">;
...
const EpisodeBrandSequenceContent = ({
    durationInFrames = DEFAULT_BRAND_SEQUENCE_DURATION,
}: EpisodeBrandSequenceContentProps) => {
    ...
    const {
        ...
        taglineY,
        taglineOpacity,
    } = getBrandSequenceState(motionFrame);

    return (
        <div className="rt-brand-sequence" aria-label="Reason Tracker brand sequence">
            <div className="rt-brand-sequence__lockup">
                <span
                    className="rt-brand-sequence__reason"
                    style={{ opacity: wordOpacity, transform: `translateX(${reasonX}px)` }}
                >
                    Reason
                </span>
                <span
                    className="rt-brand-sequence__tracker"
                    style={{ opacity: wordOpacity, transform: `translateX(${trackerX}px)` }}
                >
                    Tracker
                </span>
            </div>
...
const getScaledBrandSequenceFrame = (frame: number, duration: number) => {
    const safeDuration = Number.isFinite(duration)
        ? Math.max(1, duration)
        : DEFAULT_BRAND_SEQUENCE_DURATION;
