...
function resolveTargetBounds(
	...
	if ("component" in target) {
		return requireGraphAnimation(episode, target.component, scheduled).plan.bounds;
	}
	if ("scene" in target) {
		const visibleAnimations = resolveVisibleGraphAnimations(episode, scheduled);
		return combineBounds(visibleAnimations.map((animation) => animation.plan.bounds));
	}

	return resolveObjectsBounds(episode, scheduled, target.objects);
}
...
function resolveObjectsBounds(
	...
	if (graphKey && parsedReferences.every((reference) => reference?.graphKey === graphKey)) {
		const animation = requireGraphAnimation(episode, graphKey, scheduled);
	...
	return combineBounds(references.map((reference) =>
		resolveObjectBounds(episode, scheduled, reference)
	));
}
...
function resolveObjectBounds(
	...
	const separatorIndex = reference.indexOf(".");
	if (separatorIndex < 0) {
		return requireGraphAnimation(episode, reference, scheduled).plan.bounds;
	}
...
function requireGraphAnimation(
	...
	const animation = [...episode.graphAnimations].reverse().find((candidate) =>
		candidate.graph === graphKey && candidate.from <= scheduled.from
	);
	if (!animation) {
		throw new Error(
...
function resolveVisibleGraphAnimations(
	...
	for (const animation of episode.graphAnimations) {
		if (animation.from <= scheduled.from) {
