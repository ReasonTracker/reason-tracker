...
import { newDebate, type Debate } from "../../contracts/src/Debate.ts";
import { newScore, type ScoreId } from "../../contracts/src/Score.ts";
import { buildRecalculationWaveStep } from "../../engine/src/recalculation.ts";
import { applyStep } from "../../engine/src/step-application.ts";
...
const episode0001Debate = normalizeDebateScores(createEpisode0001Debate());
...
function normalizeDebateScores(debate: Debate): Debate {
	...
	return applyStep(debate, buildRecalculationWaveStep(debate, leafScoreIds));
}

function createEpisode0001Debate(): Debate {
	const claimIds = {
		main: asClaimId("main"),
		a: asClaimId("a"),
		b: asClaimId("b"),
	...
	return newDebate({
		id: "debate:episode-0001" as Debate["id"],
		...
		claims: {
			...
			[claimIds.a]: newClaim({ id: claimIds.a, content: "Evidence A", side: "proMain" }),
			[claimIds.b]: newClaim({ id: claimIds.b, content: "Counterpoint B", side: "proMain" }),
			[claimIds.c]: newClaim({ id: claimIds.c, content: "Sub reason C", side: "proMain" }),
			[claimIds.e]: newClaim({ id: claimIds.e, content: "Alternative source E", side: "proMain" }),
			[claimIds.f]: newClaim({ id: claimIds.f, content: "Alternative source F", side: "proMain" }),
			[claimIds.i]: newClaim({ id: claimIds.i, content: "Detail I", side: "proMain" }),
			[claimIds.l]: newClaim({ id: claimIds.l, content: "Counterpoint L", side: "conMain" }),
			[claimIds.m]: newClaim({ id: claimIds.m, content: "Support M", side: "proMain" }),
			[claimIds.n]: newClaim({ id: claimIds.n, content: "Counterpoint N", side: "conMain" }),
			[claimIds.o]: newClaim({ id: claimIds.o, content: "Counterpoint O", side: "conMain" }),
			[claimIds.u]: newClaim({ id: claimIds.u, content: "Support U", side: "conMain" }),
			[claimIds.v]: newClaim({ id: claimIds.v, content: "Support V", side: "conMain" }),
			[claimIds.w]: newClaim({ id: claimIds.w, content: "Counterpoint W", side: "proMain" }),
			[claimIds.x]: newClaim({ id: claimIds.x, content: "Support X", side: "proMain" }),
			[claimIds.y]: newClaim({ id: claimIds.y, content: "Counterpoint Y", side: "conMain" }),
			[claimIds.z]: newClaim({ id: claimIds.z, content: "Counterpoint Z", side: "proMain" }),
		},
		connectors: {
