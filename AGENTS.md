...
- For typecheck validation, run the `full-typecheck` VS Code task defined in `.vscode/tasks.json`. Do not run typecheck commands directly with `vp`.
- Run every other `vp` command from `Documents/technical/Software`, never from the repository root or a leaf package directory. For package-specific commands, target the package from that directory with `-F`.
